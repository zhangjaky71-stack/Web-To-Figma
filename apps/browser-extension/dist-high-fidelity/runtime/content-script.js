...
(() => {
    function isContentRequest(value) {
        if (typeof value !== "object" || value === null || Array.isArray(value))
            return false;
        const record = value;
        return ((record.type === "W2F_PROBE_PAGE" ||
    ...
    function pageProbe() {
        ...
        return {
            ...
            documentWidth: Math.max(root.scrollWidth, root.offsetWidth, root.clientWidth, body?.scrollWidth ?? 0, body?.offsetWidth ?? 0),
            documentHeight: Math.max(root.scrollHeight, root.offsetHeight, root.clientHeight, body?.scrollHeight ?? 0, body?.offsetHeight ?? 0),
    ...
    function openRegionSelector(jobId, sendResponse) {
    ...
    `;
        ...
        let tool = "free";
        let selectionMode = "free-rect";
        let selection = null;
        let drag = null;
        let candidate = null;
        let selectedElement = null;
        let draftExclusion = null;
        const exclusions = [];
        ...
        shadow.append(style, surface, candidateBox, selectionBox, redactionLayer, sizeLabel, toolbar, hint);
        document.documentElement.append(host);
        function documentBounds() {
            const probe = pageProbe();
            return { x: 0, y: 0, width: probe.documentWidth, height: probe.documentHeight };
        }
        function documentPoint(clientX, clientY) {
        ...
        function snapPoint(clientX, clientY, point, bypass) {
            if (bypass)
                return point;
            const element = elementAt(clientX, clientY);
            if (!element)
        ...
        function setFixedRect(element, bounds) {
            if (!bounds) {
        ...
        function updateDrag(event) {
            ...
            const current = drag.kind === "selection" && tool === "free"
        ...
        function autoScrollVelocity(position, size) {
            const edge = 56;
            const maxSpeed = 28;
            if (position < edge)
                return -maxSpeed * (1 - Math.max(0, position) / edge);
        ...
        function selectionRoot() {
            ...
            if (!element)
                element = elementAt(lastPointerClient.x, lastPointerClient.y);
        ...
        function moveSelection(dx, dy) {
            ...
            let nextDx = dx;
            let nextDy = dy;
            if (selection.x + nextDx < 0)
                nextDx = -selection.x;
            if (selection.y + nextDy < 0)
                nextDy = -selection.y;
        ...
        surface.addEventListener("pointerdown", (event) => {
            ...
            const kind = tool === "redact" ? "redact" : tool === "exclude" ? "exclude" : "selection";
            drag = {
    ...
    const shellGlobal = globalThis;
    if (shellGlobal.__W2F_CONTENT_SHELL_INSTALLED__)
        return;
    ...
    chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
        ...
        if (message.type === "W2F_CANCEL_REGION_SELECTION") {
            if (activeSession?.jobId === message.jobId)
                activeSession.cancel();
