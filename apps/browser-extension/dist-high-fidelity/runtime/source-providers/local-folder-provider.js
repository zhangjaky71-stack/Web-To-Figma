...
import { buildLocalFolderLocator, getUrlProtocol, normalizeLocalRelativePath, resolveUrlReference, splitReferenceSuffix, } from "./urls.js";
function normalizedEntries(entries) {
    ...
    for (const entry of entries) {
        ...
        if (result.has(relativePath)) {
            throw new SourceProviderError("invalid-local-folder-selection", `Duplicate local folder entry: ${relativePath}`);
        }
        result.set(relativePath, { ...entry, relativePath });
    }
    return result;
}
function getSelectionError(input) {
...
function joinLocalReference(documentPath, referencePath) {
    if (!referencePath)
        return documentPath;
...
//# sourceMappingURL=local-folder-provider.js.map