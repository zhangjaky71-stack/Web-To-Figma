...
function tree(boundaryReasons: string[], includeText = true): WtfRenderTree {
  return {
    ...
    nodes: [
      ...
      node("boundary", "root", includeText ? ["label"] : [], {
        strategy: "raster",
...
describe("NODE-31 raster text authorization policy", () => {
  it("is not applicable when a raster boundary has no ordinary text", () => {
    expect(
      evaluateRasterTextPolicy(tree(["canvas fallback"], false), "boundary", "balanced"),
    ).toMatchObject({ status: "not-applicable", textRenderNodeIds: [] });
  });

  it.each(["balanced", "high-fidelity"] as const)(
    "authorizes text carried by a mix-blend compositing boundary in %s",
    (profile) => {
      const decision = evaluateRasterTextPolicy(
        tree(["mix-blend-mode depends on sibling/ancestor backdrop pixels"]),
        "boundary",
        profile,
      );
  ...
  it("preserves ordinary text in design-friendly profile even with a visual dependency", () => {
    const decision = evaluateRasterTextPolicy(
      tree(["backdrop-filter samples pixels behind the filtered node"]),
      "boundary",
      "design-friendly",
    );
  ...
  it("does not authorize raster text for font, geometry or pixel-score reasons alone", () => {
    const decision = evaluateRasterTextPolicy(
      tree([
        ...
        "geometry correction drift remains above tolerance",
  ...
  it("does not mistake an unsupported font reason for an unsupported visual dependency", () => {
    const decision = evaluateRasterTextPolicy(
      tree(["unsupported font family requested by source text"]),
      "boundary",
      "high-fidelity",
    );
    expect(decision.status).toBe("native-preserved");
  ...
  it("uses the visual cause, not a font diagnostic, when both are present", () => {
    const decision = evaluateRasterTextPolicy(
      tree(["unsupported blend effect requires visual fallback", "font substitution mismatch"]),
      "boundary",
      "balanced",
    );
    expect(decision.status).toBe("raster-authorized");
    expect(decision.visualJustifications).toEqual([
      "unsupported blend effect requires visual fallback",
    ]);
  ...
  it("authorizes a promoted mask group when flattening is required for compositing", () => {
    const decision = evaluateRasterTextPolicy(
      tree(["mask requires the affected subtree to be flattened as one compositing group"]),
      "boundary",
      "high-fidelity",
    );
    expect(decision.status).toBe("raster-authorized");
