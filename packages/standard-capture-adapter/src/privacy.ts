...
export function isSensitiveCapturedAttribute(tagName: string, attributeName: string): boolean {
  ...
  if (name === "srcdoc" || name === "style" || name.startsWith("on")) return true;
  if ((tag === "INPUT" || tag === "TEXTAREA") && name === "value") return true;
  return SENSITIVE_ATTRIBUTE_PATTERN.test(name);
}

export function sanitizeCapturedUrl(value: string, baseUrl: string): string {
  try {
...
export function sanitizeCapturedAttributes(
  tagName: string,
