...
function add(errors: WtfIrValidationError[], path: string, code: string, message: string): void {
...
function validateConfidence(value: unknown, errors: WtfIrValidationError[], path: string): void {
...
function validateGeometry(value: unknown, errors: WtfIrValidationError[], path: string): void {
  if (!isRecord(value)) {
  ...
  if (!boundsResult.ok) {
    for (const error of boundsResult.errors) {
      add(
        errors,
        `${path}.bounds${error.path === "$" ? "" : error.path.slice(1)}`,
        error.code,
        error.message,
      );
    }
  ...
  if (value.paintOrder !== undefined && !Number.isSafeInteger(value.paintOrder)) {
    add(
      errors,
      `${path}.paintOrder`,
      "WTF_IR_PAINT_ORDER_INVALID",
      "paintOrder must be an integer",
    );
  }
}
...
function validateSourceGraph(
  ...
  if (!isRecord(value) || !Array.isArray(value.nodes)) {
    add(
      errors,
      "$.sourceGraph",
      "WTF_IR_SOURCE_GRAPH_INVALID",
      "sourceGraph.nodes must be an array",
    );
  ...
  value.nodes.forEach((nodeValue, index) => {
    ...
    if (!isNonEmptyString(id)) {
      add(
        errors,
        `${path}.captureNodeId`,
        "WTF_IR_SOURCE_ID_INVALID",
        "captureNodeId must be non-empty",
      );
      return;
    }
    if (sourceIds.has(id)) {
    ...
    const children = Array.isArray(nodeValue.childCaptureNodeIds)
      ...
      : [];
    if (
      !Array.isArray(nodeValue.childCaptureNodeIds) ||
      children.length !== nodeValue.childCaptureNodeIds.length
    ) {
      add(
        errors,
        `${path}.childCaptureNodeIds`,
        "WTF_IR_SOURCE_CHILDREN_INVALID",
      ...
      for (const assetId of nodeValue.assetRefs) {
        if (isNonEmptyString(assetId)) assetRefs.add(assetId);
          ...
          add(
            errors,
            `${path}.assetRefs`,
            "WTF_IR_ASSET_REF_INVALID",
            "asset refs must be strings",
          );
      }
    }
    ...
    if (stableIdentity !== undefined) {
      if (!isRecord(stableIdentity) || !isNonEmptyString(stableIdentity.id)) {
        add(
          errors,
          `${path}.stableIdentity`,
          "WTF_IR_STABLE_ID_INVALID",
          "stable identity must have an id",
        );
      } else {
        if (stableIds.has(stableIdentity.id)) {
          add(
            errors,
            `${path}.stableIdentity.id`,
            "WTF_IR_STABLE_ID_DUPLICATE",
            `duplicate stable id: ${stableIdentity.id}`,
          );
        }
    ...
    if (fingerprint !== undefined && isRecord(fingerprint)) {
      validateConfidence(
        ...
        errors,
        `${path}.structuralFingerprint.confidence`,
      );
    }
  });
  ...
  if (!isNonEmptyString(value.rootCaptureNodeId)) {
    add(
      errors,
      "$.sourceGraph.rootCaptureNodeId",
      "WTF_IR_SOURCE_ROOT_INVALID",
      "source root must be non-empty",
    );
  } else {
    ...
    for (const field of [
      ...
      if (ref !== undefined && (!isNonEmptyString(ref) || !sourceIds.has(ref))) {
        add(
          errors,
          `${path}.${field}`,
          "WTF_IR_SOURCE_RELATION_MISSING",
          `${field} must reference a source node`,
        );
      }
    }
  }
  ...
  if (!Array.isArray(value.scrollContainers)) {
    add(
      errors,
      "$.sourceGraph.scrollContainers",
      "WTF_IR_SCROLL_CONTAINERS_INVALID",
      "scrollContainers must be an array",
    );
  } else {
    value.scrollContainers.forEach((item, index) => {
      ...
      if (
        ...
        !sourceIds.has(item.sourceNodeId)
      ) {
        add(
          errors,
          `${path}.sourceNodeId`,
          "WTF_IR_SCROLL_NODE_MISSING",
          "scroll container must reference a source node",
        );
        return;
      }
      for (const field of [
        ...
        if (!isFiniteNumber(item[field])) {
          add(
            errors,
            `${path}.${field}`,
            "WTF_IR_SCROLL_GEOMETRY_INVALID",
  ...
  if (!isRecord(value.revision)) {
    ...
    for (const field of [
      ...
      if (!isNonEmptyString(value.revision[field])) {
        add(
          errors,
          `$.sourceGraph.revision.${field}`,
          "WTF_IR_REVISION_FIELD_INVALID",
...
function validateRenderTree(
  ...
  value.nodes.forEach((nodeValue, index) => {
    ...
    if (renderIds.has(nodeValue.id)) {
      add(
        errors,
        `${path}.id`,
        "WTF_IR_RENDER_ID_DUPLICATE",
        `duplicate render id: ${nodeValue.id}`,
      );
      return;
    }
    renderIds.add(nodeValue.id);
    const children = Array.isArray(nodeValue.childIds)
      ? nodeValue.childIds.filter(isNonEmptyString)
      : [];
    ...
    if (!Array.isArray(nodeValue.sourceNodeIds) || nodeValue.sourceNodeIds.length === 0) {
      add(
        errors,
        `${path}.sourceNodeIds`,
        "WTF_IR_RENDER_SOURCE_EMPTY",
        "render nodes must map to source nodes",
      );
    } else {
      for (const sourceId of nodeValue.sourceNodeIds) {
        if (!isNonEmptyString(sourceId) || !sourceIds.has(sourceId)) {
          add(
            errors,
            `${path}.sourceNodeIds`,
            "WTF_IR_RENDER_SOURCE_MISSING",
            `unknown source node: ${String(sourceId)}`,
          );
        }
      }
    }
    ...
    if (isRecord(nodeValue.renderDecision)) {
      validateConfidence(
        nodeValue.renderDecision.confidence,
        errors,
        `${path}.renderDecision.confidence`,
      );
    } else {
      add(
        errors,
        `${path}.renderDecision`,
        "WTF_IR_RENDER_DECISION_INVALID",
        "render decision is required",
      );
    }
    if (isRecord(nodeValue.layout) && isRecord(nodeValue.layout.decision)) {
      validateConfidence(
        nodeValue.layout.decision.confidence,
        errors,
        `${path}.layout.decision.confidence`,
      );
    } else {
      add(errors, `${path}.layout`, "WTF_IR_LAYOUT_INVALID", "layout decision is required");
    }
    if (
      !isRecord(nodeValue.paint) ||
      !isFiniteNumber(nodeValue.paint.opacity) ||
      nodeValue.paint.opacity < 0 ||
      nodeValue.paint.opacity > 1
    ) {
      add(
        errors,
        `${path}.paint.opacity`,
        "WTF_IR_OPACITY_INVALID",
        "paint opacity must be within 0..1",
      );
    }
    if (Array.isArray(nodeValue.assetRefs)) {
      for (const assetId of nodeValue.assetRefs) {
  ...
  if (!isNonEmptyString(value.rootId)) {
    add(
      errors,
      "$.renderTree.rootId",
      "WTF_IR_RENDER_ROOT_INVALID",
      "render root must be non-empty",
    );
  } else {
  ...
  for (const [index, nodeValue] of value.nodes.entries()) {
    ...
    if (
      nodeValue.parentId !== undefined &&
      (!isNonEmptyString(nodeValue.parentId) || !renderIds.has(nodeValue.parentId))
    ) {
      add(
        errors,
        `$.renderTree.nodes[${index}].parentId`,
        "WTF_IR_RENDER_PARENT_MISSING",
        "parentId must reference a render node",
      );
    }
  }
...
export function validateWtfIrBundle(value: unknown): WtfIrValidationResult<WtfIrBundle> {
  ...
  if (!isRecord(value)) {
    return {
      ok: false,
      errors: [
        { path: "$", code: "WTF_IR_BUNDLE_INVALID", message: "IR bundle must be an object" },
      ],
    };
  }
  ...
  if (!isRecord(document)) {
    ...
    if (document.irVersion !== WTF_IR_VERSION) {
      add(
        ...
        `IR version must be ${WTF_IR_VERSION}`,
      );
    }
    for (const field of [
      "documentId",
  ...
  const styleIds =
    isRecord(value.styles) && Array.isArray(value.styles.styles)
      ? collectUniqueIds(
          value.styles.styles,
          (item) => item.id,
          errors,
          "$.styles.styles",
          "WTF_IR_STYLE_ID_INVALID",
        )
      : new Set<string>();
  ...
  for (const styleRef of source.styleRefs) {
    if (!styleIds.has(styleRef))
      add(errors, "$.sourceGraph", "WTF_IR_STYLE_REF_MISSING", `unknown style ref: ${styleRef}`);
  }

  const assetIds =
    isRecord(value.assets) && Array.isArray(value.assets.assets)
      ? collectUniqueIds(
          value.assets.assets,
          (item) => item.id,
          errors,
          "$.assets.assets",
          "WTF_IR_ASSET_ID_INVALID",
        )
      : new Set<string>();
  if (
    !isRecord(value.assets) ||
    !Array.isArray(value.assets.assets) ||
    !Array.isArray(value.assets.referenceTiles)
  ) {
    add(
      errors,
      "$.assets",
      "WTF_IR_ASSETS_INVALID",
      "assets payload must contain assets/referenceTiles arrays",
    );
  } else {
    value.assets.assets.forEach((asset, index) => {
      ...
      if (
        ...
        add(
          errors,
          `$.assets.assets[${index}].sha256`,
          "WTF_IR_ASSET_HASH_INVALID",
          "asset sha256 must be canonical",
        );
      }
    });
  }
  for (const assetRef of [...source.assetRefs, ...render.assetRefs]) {
    if (!assetIds.has(assetRef))
      add(errors, "$.assets", "WTF_IR_ASSET_REF_MISSING", `unknown asset ref: ${assetRef}`);
  }

  const stateIds =
    isRecord(value.states) && Array.isArray(value.states.states)
      ? collectUniqueIds(
          value.states.states,
          (item) => item.id,
          errors,
          "$.states.states",
          "WTF_IR_STATE_ID_INVALID",
        )
      : new Set<string>();
  if (!isRecord(value.states) || !Array.isArray(value.states.states)) {
    ...
    value.states.states.forEach((state, index) => {
      if (
        !isRecord(state) ||
        !isNonEmptyString(state.rootNodeId) ||
        !source.sourceIds.has(state.rootNodeId)
      ) {
        add(
          errors,
          `$.states.states[${index}].rootNodeId`,
          "WTF_IR_STATE_ROOT_MISSING",
          "state root must reference a source node",
        );
      }
    });
  }
  ...
  if (isRecord(document) && Array.isArray(document.environments)) {
    for (const [index, environment] of document.environments.entries()) {
      ...
      if (environmentIds.has(environment.id))
        add(
          errors,
          `${path}.id`,
          "WTF_IR_ENV_ID_DUPLICATE",
          `duplicate environment id: ${environment.id}`,
        );
    ...
    if (!Array.isArray(document.environmentRefs)) {
      add(
        errors,
        "$.document.environmentRefs",
        "WTF_IR_ENV_REFS_INVALID",
        "environmentRefs must be an array",
      );
    } else {
      for (const ref of document.environmentRefs) {
        if (!isNonEmptyString(ref) || !environmentIds.has(ref)) {
          add(
            errors,
            "$.document.environmentRefs",
            "WTF_IR_ENV_REF_MISSING",
            `unknown environment ref: ${String(ref)}`,
          );
        }
      }
    }
  ...
  if (
    ...
    !Array.isArray(value.responsive.snapshots) ||
    !Array.isArray(value.responsive.rules) ||
    !Array.isArray(value.responsive.mediaRules) ||
    !Array.isArray(value.responsive.containerQueries)
  ) {
    add(
      errors,
      "$.responsive",
      "WTF_IR_RESPONSIVE_INVALID",
      "responsive payload arrays are required",
    );
  } else {
    const snapshotIds = collectUniqueIds(
      value.responsive.snapshots,
      (item) => item.id,
      errors,
      "$.responsive.snapshots",
      "WTF_IR_SNAPSHOT_ID_INVALID",
    );
    value.responsive.snapshots.forEach((snapshot, index) => {
      ...
      if (!isNonEmptyString(snapshot.rootNodeId) || !source.sourceIds.has(snapshot.rootNodeId)) {
        add(
          errors,
          `${path}.rootNodeId`,
          "WTF_IR_SNAPSHOT_ROOT_MISSING",
          "snapshot root must reference a source node",
        );
      }
      if (
        !isNonEmptyString(snapshot.environmentRef) ||
        !environmentIds.has(snapshot.environmentRef)
      ) {
        add(
          errors,
          `${path}.environmentRef`,
          "WTF_IR_SNAPSHOT_ENV_MISSING",
          "snapshot environment must exist",
        );
      }
      if (
        snapshot.stateRef !== undefined &&
        (!isNonEmptyString(snapshot.stateRef) || !stateIds.has(snapshot.stateRef))
      ) {
        add(
          errors,
          `${path}.stateRef`,
          "WTF_IR_SNAPSHOT_STATE_MISSING",
          "snapshot state must exist",
        );
      }
    });
    value.responsive.rules.forEach((rule, index) => {
      ...
      if (
        !isNonEmptyString(rule.targetStableNodeId) ||
        !source.stableIds.has(rule.targetStableNodeId)
      ) {
        add(
          errors,
          `${path}.targetStableNodeId`,
          "WTF_IR_RESPONSIVE_TARGET_MISSING",
          "responsive target stable id must exist",
        );
      }
          ...
          for (const snapshotId of range.snapshotIds) {
            if (!isNonEmptyString(snapshotId) || !snapshotIds.has(snapshotId)) {
              add(
                errors,
                `${path}.ranges`,
                "WTF_IR_RESPONSIVE_SNAPSHOT_MISSING",
                `unknown snapshot id: ${String(snapshotId)}`,
              );
            }
          }
        }
  ...
  const diagnosticIds =
    isRecord(value.diagnostics) && Array.isArray(value.diagnostics.diagnostics)
      ? collectUniqueIds(
          value.diagnostics.diagnostics,
          (item) => item.id,
          errors,
          "$.diagnostics.diagnostics",
          "WTF_IR_DIAGNOSTIC_ID_INVALID",
        )
      : new Set<string>();
  if (!isRecord(value.diagnostics) || !Array.isArray(value.diagnostics.diagnostics)) {
    add(
      errors,
      "$.diagnostics",
      "WTF_IR_DIAGNOSTICS_INVALID",
      "diagnostics payload must contain an array",
    );
  } else {
    value.diagnostics.diagnostics.forEach((diagnostic, index) => {
      ...
      if (Array.isArray(diagnostic.sourceNodeIds)) {
        for (const id of diagnostic.sourceNodeIds) {
          if (!isNonEmptyString(id) || !source.sourceIds.has(id))
            add(
              errors,
              `${path}.sourceNodeIds`,
              "WTF_IR_DIAGNOSTIC_SOURCE_MISSING",
              `unknown source id: ${String(id)}`,
            );
        }
      }
      if (Array.isArray(diagnostic.renderNodeIds)) {
        for (const id of diagnostic.renderNodeIds) {
          if (!isNonEmptyString(id) || !render.renderIds.has(id))
            add(
              errors,
              `${path}.renderNodeIds`,
              "WTF_IR_DIAGNOSTIC_RENDER_MISSING",
              `unknown render id: ${String(id)}`,
            );
        }
      }
    });
  ...
  for (const diagnosticRef of render.diagnosticRefs) {
    if (!diagnosticIds.has(diagnosticRef))
      add(
        errors,
        "$.renderTree",
        "WTF_IR_DIAGNOSTIC_REF_MISSING",
        `unknown diagnostic ref: ${diagnosticRef}`,
      );
  }
  ...
  if (!tokenResult.ok) {
    for (const error of tokenResult.errors) {
      add(
        errors,
        `$.tokens${error.path === "$" ? "" : error.path.slice(1)}`,
        error.code,
        error.message,
      );
    }
  ...
  if (isRecord(document) && isRecord(value.sourceGraph) && isRecord(value.renderTree)) {
    if (document.sourceGraphRootId !== value.sourceGraph.rootCaptureNodeId) {
      add(
        errors,
        "$.document.sourceGraphRootId",
        "WTF_IR_SOURCE_ROOT_MISMATCH",
        "document/source graph roots differ",
      );
    }
    if (document.renderTreeRootId !== value.renderTree.rootId) {
      add(
        errors,
        "$.document.renderTreeRootId",
        "WTF_IR_RENDER_ROOT_MISMATCH",
        "document/render tree roots differ",
      );
    }
      ...
      for (const field of ["documentId", "captureId", "revisionId", "sourceFingerprint"] as const) {
        if (document[field] !== value.sourceGraph.revision[field]) {
          add(
            errors,
            `$.document.${field}`,
            "WTF_IR_REVISION_MISMATCH",
            `${field} differs from source revision`,
          );
        }
      }
    }
