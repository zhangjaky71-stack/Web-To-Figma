...
const outputRoot = resolve(
  process.argv[3] ??
    process.env.W2F_NODE31_DESKTOP_EVIDENCE_OUT ??
    "artifacts/node31-desktop-evidence",
);
...
assert(
  inputPath,
  "Usage: node scripts/ingest-node-31-desktop-evidence.mjs <archive.json> [output-dir]",
);
...
assert(
  typeof archive.sampleId === "string" && archive.sampleId.length > 0,
  "Archive sampleId missing",
);
assert(/^[a-f0-9]{64}$/.test(archive.sourceSha256), "Archive sourceSha256 is invalid");
assert(
  Array.isArray(archive.files) && archive.files.length >= 4,
  "Desktop evidence archive is incomplete",
);
...
for (const entry of archive.files) {
  ...
  assert(
    ...
    `Unsafe archive filename: ${entry.name}`,
  );
...
const measurementEntries = [...files.values()].filter(({ entry }) =>
  entry.name.endsWith(".measurement.json"),
);
const renderEntries = [...files.values()].filter(({ entry }) =>
  entry.name.endsWith(".figma-render.json"),
);
const exportEntries = [...files.values()].filter(({ entry }) =>
  entry.name.endsWith(".figma-export.json"),
);
...
assert(
  measurement.sample?.sourceSha256 === archive.sourceSha256,
  "Archive/measurement source hash mismatch",
);
assert(measurement.pipeline?.figmaRender?.status === "PASS", "Figma render stage is not PASS");
assert(
  measurement.pipeline?.figmaRender?.host?.kind === "figma-desktop",
  "Render host is not figma-desktop",
);
assert(measurement.pipeline?.figmaExport?.status === "PASS", "Figma export stage is not PASS");
assert(
  measurement.pipeline.figmaRender.artifact === renderFile.entry.name,
  "Render artifact path mismatch",
);
assert(
  measurement.pipeline.figmaRender.sha256 === renderFile.entry.sha256,
  "Render artifact hash mismatch",
);
assert(
  measurement.pipeline.figmaExport.artifact === exportFile.entry.name,
  "Export artifact path mismatch",
);
assert(
  measurement.pipeline.figmaExport.sha256 === exportFile.entry.sha256,
...
assert(
  exportManifest.evidenceType === "node31-figma-desktop-export",
  "Export evidence type mismatch",
);
assert(exportManifest.sampleId === archive.sampleId, "Export evidence sample mismatch");
assert(
  exportManifest.sourceSha256 === archive.sourceSha256,
  "Export evidence source hash mismatch",
);
assert(
  Array.isArray(exportManifest.tiles) && exportManifest.tiles.length === pngEntries.length,
  "Export tile manifest mismatch",
);
