...
async function driverJson(url, init) {
  ...
  if (!response.ok || payload?.value?.error) {
    throw new Error(
      `NODE-31 V12 ChromeDriver helper failed: ${response.status} ${JSON.stringify(payload)}`,
    );
  }
...
async function enableUserModifiableExtension(sessionBase, extensionId) {
  return driverJson(`${sessionBase}/execute/async`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
...
async function waitForEnabledRegistry(sessionBase, extensionId) {
  let lastInfo = null;
  ...
  throw new Error(
    `NODE-31 V12 extension did not enter ENABLED + active file access state: ${JSON.stringify(lastInfo)}`,
  );
}
...
async function openAuthorizedOptions(sessionId, extensionId) {
  ...
  if (!management.handle) {
    ...
    throw new Error(
      `NODE-31 V12 could not locate chrome://extensions handle: ${JSON.stringify(management.diagnostics)}`,
    );
  }

  let registryInfo = await readRegistryInfo(sessionBase, extensionId);
  console.log(
    `NODE-31 file protocol v12: pre-enable registry diagnostics ${JSON.stringify(registryInfo)}`,
  );
  if (registryInfo?.ok !== true) {
    throw new Error(`NODE-31 V12 cannot read extension registry state: ${registryInfo?.error}`);
  }

  if (registryInfo.state !== "ENABLED") {
    if (registryInfo.state !== "DISABLED" || registryInfo.userMayModify !== true) {
      throw new Error(
        `NODE-31 V12 refuses non-user-authorized registry mutation: ${JSON.stringify(registryInfo)}`,
      );
    }
    console.log(
      `NODE-31 file protocol v12: enabling user-modifiable BiDi extension ${extensionId} through chrome.management`,
    );
    const enableResult = await enableUserModifiableExtension(sessionBase, extensionId);
    if (enableResult?.ok !== true) {
      throw new Error(`NODE-31 V12 chrome.management.setEnabled failed: ${enableResult?.error}`);
    }
    registryInfo = await waitForEnabledRegistry(sessionBase, extensionId);
    console.log(
      `NODE-31 file protocol v12: post-enable registry diagnostics ${JSON.stringify(registryInfo)}`,
    );
  }

  if (registryInfo?.fileAccess?.isActive !== true) {
    throw new Error("NODE-31 V12 explicit file access is not active before opening options");
  }
  ...
  if (showResult?.ok !== true) {
    await switchWindow(sessionBase, originalHandle).catch(() => undefined);
    throw new Error(`NODE-31 V12 developerPrivate.showOptions failed: ${showResult?.error}`);
  }
  ...
  for (let attempt = 0; attempt < 400; attempt += 1) {
    ...
    if (extensionTarget.handle) {
      console.log(
        `NODE-31 file protocol v12: Chrome opened authorized extension target ${extensionTarget.handle} ${extensionTarget.url}`,
      );
  ...
  await switchWindow(sessionBase, originalHandle).catch(() => undefined);
  throw new Error(
    `NODE-31 V12 showOptions did not expose an authorized extension target: ${JSON.stringify(lastDiagnostics)}`,
  );
}
...
globalThis.fetch = async (input, init = {}) => {
  ...
  if (url === "http://127.0.0.1:9515/session" && init.method === "POST" && init.body) {
    ...
    if (!chromeOptions || typeof chromeOptions !== "object") {
      throw new Error("NODE-31 V12 could not locate goog:chromeOptions in ChromeDriver session request");
    }
    chromeOptions.enableExtensionTargets = true;
    init = { ...init, body: JSON.stringify(body) };
    console.log("NODE-31 file protocol v12: enabling ChromeDriver extension target introspection");
  }
  ...
  if (navigationMatch && init.method === "POST" && init.body) {
    ...
    if (extensionMatch) {
      ...
      console.log(
        `NODE-31 file protocol v12: replacing synthetic extension navigation with Chrome-owned showOptions(${extensionId})`,
      );
