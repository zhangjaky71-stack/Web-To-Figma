...
async function driverJson(url, init) {
  ...
  if (!response.ok || payload?.value?.error) {
    throw new Error(`NODE-31 V8 ChromeDriver helper failed: ${response.status} ${JSON.stringify(payload)}`);
  }
...
async function openAuthorizedOptions(sessionId, extensionId) {
  ...
  if (!management.handle) {
    ...
    throw new Error(
      `NODE-31 V8 could not locate chrome://extensions handle: ${JSON.stringify(management.diagnostics)}`,
    );
  }
  ...
  if (showResult?.ok !== true) {
    await switchWindow(sessionBase, originalHandle).catch(() => undefined);
    throw new Error(`NODE-31 V8 developerPrivate.showOptions failed: ${showResult?.error}`);
  }
  ...
  for (let attempt = 0; attempt < 400; attempt += 1) {
    ...
    if (extensionTarget.handle) {
      console.log(
        `NODE-31 file protocol v8: Chrome opened authorized extension target ${extensionTarget.handle} ${extensionTarget.url}`,
      );
  ...
  await switchWindow(sessionBase, originalHandle).catch(() => undefined);
  throw new Error(
    `NODE-31 V8 showOptions did not expose an authorized extension target: ${JSON.stringify(lastDiagnostics)}`,
  );
}
...
globalThis.fetch = async (input, init = {}) => {
  ...
  if (url === "http://127.0.0.1:9515/session" && init.method === "POST" && init.body) {
    ...
    if (!chromeOptions || typeof chromeOptions !== "object") {
      throw new Error("NODE-31 V8 could not locate goog:chromeOptions in ChromeDriver session request");
    }
    chromeOptions.enableExtensionTargets = true;
    init = { ...init, body: JSON.stringify(body) };
    console.log("NODE-31 file protocol v8: enabling ChromeDriver extension target introspection");
  }
  ...
  if (navigationMatch && init.method === "POST" && init.body) {
    ...
    if (extensionMatch) {
      ...
      console.log(
        `NODE-31 file protocol v8: replacing synthetic extension navigation with developerPrivate.showOptions(${extensionId})`,
      );
