...
for (const file of requiredFiles)
  assert(existsSync(resolve(root, file)), `NODE-21 missing ${file}`);
...
if (failures.length === 0) {
  ...
  assert(
    ...
    "WTF Packager must consume shared schema",
  );
  ...
  assert(
    browserPackage.dependencies?.["@w2f/wtf-packager"] === "workspace:*",
    "Browser must depend on wtf-packager",
  );
  for (const script of [
    "build",
    "build:standard",
    ...
    assert(
      browserPackage.scripts?.[script]?.includes("validate-node-21-package.mjs"),
      `Browser ${script} must require NODE-21 package validation`,
    );
  }

  const packageExtension = readText("apps/browser-extension/scripts/package-extension.mjs");
  assert(
    packageExtension.includes('specifier: "@w2f/wtf-packager"') &&
      packageExtension.includes('directory: "wtf-packager"'),
    "Browser packager must ship WTF Packager runtime",
  );
  ...
  for (const manifestPath of [
    ...
    assert(
      ...
      `${manifestPath} must include downloads permission`,
    );
  }
  ...
  assert(
    protocol.includes("WtfExportReceipt"),
    "Browser protocol must expose WtfExportReceipt response data",
  );

  const exportContract = readText("apps/browser-extension/src/runtime/wtf-export-contract.ts");
  assert(
    exportContract.includes("isWtfExportReceipt"),
    "Browser export receipt contract must be runtime-validatable",
  );
...
if (failures.length > 0) {
  console.error(
    `NODE-21 foundation validation failed:\n${failures.map((item) => `- ${item}`).join("\n")}`,
  );
  process.exitCode = 1;
} else {
