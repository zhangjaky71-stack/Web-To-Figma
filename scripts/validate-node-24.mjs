...
if (failures.length === 0) {
  ...
  assert(
    ...
    "resolver must consume W2F IR through workspace-only resolution",
  );
  assert(
    !pkg.dependencies?.["@figma/plugin-typings"] && !pkg.devDependencies?.["@figma/plugin-typings"],
    "resolver policy package must not hard-bind Figma typings",
  );
  ...
  for (const evidence of [
    ...
    "balanced",
    "design-friendly",
    "preservesRevisionMetadata",
    "preservesStableSourceMapping",
    'tokenPolicy: "literal"',
  ])
  ...
  for (const evidence of [
    ...
    "ImagePaint.imageTransform:CROP",
  ])
  ...
  assert(
    policy.includes('profile === "high-fidelity" ? "fidelity"'),
    "NODE-24 must normalize the NODE-22 Fidelity transport id",
  );
  ...
  for (const forbidden of ["figma.", "@figma/plugin-typings", "createFrame(", "createRectangle("]) {
    assert(
      !policy.includes(forbidden) && !resolver.includes(forbidden),
      `NODE-24 policy must not create or directly probe Figma nodes: ${forbidden}`,
    );
  }
  ...
  assert(
    nodeDoc.includes("NODE-28 executes raster fallback"),
    "NODE-24/NODE-28 raster boundary must remain explicit",
  );
}

if (failures.length > 0) {
  console.error(
    `NODE-24 foundation validation failed:\n${failures.map((item) => `- ${item}`).join("\n")}`,
  );
  process.exitCode = 1;
} else {
